...
export const generateAppOnServer = createServerFn({ method: "POST" })
	...
	.handler(async ({ data: appDescription }: { data: string }) => {
		if (!env.VITE_OPENAI_API_KEY) {
			throw new Error(
				"OpenAI API key is not configured. Please set VITE_OPENAI_API_KEY in your .env.local file.",
			);
		}
...
export const generateAppRefinementOnServer = createServerFn({ method: "POST" })
	.validator((data: { currentCode: string; refinementMessage: string }) => {
		if (
			typeof data.currentCode !== "string" ||
			data.currentCode.trim() === ""
		) {
			throw new Error("Current app code cannot be empty.");
		}
		if (
			typeof data.refinementMessage !== "string" ||
			data.refinementMessage.trim() === ""
		) {
			throw new Error("Refinement message cannot be empty.");
		}
		return data;
	})
	.handler(
		async ({
			...
			data: { currentCode: string; refinementMessage: string };
		}) => {
			if (!env.VITE_OPENAI_API_KEY) {
				throw new Error(
					"OpenAI API key is not configured. Please set VITE_OPENAI_API_KEY in your .env.local file.",
			...
			for (let attempt = 1; attempt <= maxRetries; attempt++) {
				try {
					...
					lastError = error instanceof Error ? error : new Error(String(error));
					console.error(
						`❌ AI refinement attempt ${attempt} failed:`,
						lastError.message,
					);
			...
			throw (
				lastError ||
				new Error("An unexpected error occurred while refining the app.")
			);
		},
	);
